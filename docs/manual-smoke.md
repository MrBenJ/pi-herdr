...
**Status: partially run (2026-09-18, owner-driven, Herdr 0.9.0, Pi 0.85.1, pi-profile 0.1.0, macOS, zsh).** In a disposable repository with a disposable profile, six profiled launches were made. The first exposed a result-layer bug (Herdr's silent `pane run` acknowledgement was reported as a `profile-run` failure although the worker had started); after the fix, all five further launches succeeded end to end (one before the profile had a model login, so its prompt was submitted but unanswered; four with a login, three of them back to back): one no-focus tab, foreground processes `pi-profile --cwd <worktree> <profile>` plus its `pi` child, the profile indicator shown, the requested name confirmed on the exact pane, and, in all four logged-in launches, a three-line prompt delivered once and intact with the boundary and answered correctly by the worker. No prompt was dropped or split under the two-idle readiness rule. `agent_session`, `interactive_ready`, and `launch_pending` were absent for the profiled worker. Not yet run: steps 1, 6–8, and the default-directory variant of step 3.
