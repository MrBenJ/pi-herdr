...
Profiled launches (`piProfile`) additionally depend on `pane run` accepting one command string, `agent get <pane-id>` failing with error code `agent_not_found` while no agent is detected, `agent_info.agent.{agent,agent_status,pane_id,name}`, and `agent rename <pane-id> <name>`. Read-only observation on Herdr 0.9.0 (macOS) showed a pane whose foreground processes were `node …/pi-profile <name>` and its `pi` child reported as agent `pi`, which is the detection this path relies on; the full launch sequence still requires the owner-authorized manual smoke. `pi-profile` 0.1.0 needs no change: everything after the profile name is forwarded to Pi unchanged.
