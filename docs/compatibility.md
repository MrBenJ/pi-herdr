...
- Real isolated pi-loader tests load both package extension entry points twice and dispose them repeatedly without hosting context, a model/provider call, or runtime subprocess/capture creation. Packed-source loading also omits the package's devDependencies.
- Earlier planning performed read-only hosting `workspace list` and an explicit nonexistent-socket probe to verify selection/no fallback. Orchestrator implementation also performed read-only `workspace list` and `pane list --workspace wG` to confirm list envelopes/CWD fields. `--version`, `--help`, and `api schema --json` are local CLI research, not live launch tests. No real workspace/tab/pane/agent was created, focused or closed by this implementation work. Manual launch smoke remains unrun.

## Orchestrator compatibility

The orchestrator requires Git commands available in current supported Git releases: `rev-parse --show-toplevel`, `rev-parse --git-common-dir`, `symbolic-ref`, `show-ref`, `check-ignore`, `check-ref-format --branch`, `worktree list --porcelain`, and `worktree add`. It assumes a non-bare main checkout whose common directory is its `.git` directory. Linked worktrees are valid pane CWDs for workspace identity but invalid as caller-supplied `repoRoot` values.

Herdr 0.8.2 response dependencies are `workspace_list.workspaces[].workspace_id`, `pane_list.panes[].{pane_id,workspace_id,cwd}`, `workspace_created.workspace.workspace_id`, `tab_created.tab.tab_id`, `tab_created.root_pane.pane_id`, and `agent_started`/`agent_prompted` agent identity fields. Incomplete, truncated, or shape-incompatible topology results fail closed.

Profiled launches (`piProfile`) additionally depend on `pane run` accepting one command string, `agent get <pane-id>` failing with error code `agent_not_found` while no agent is detected, `agent_info.agent.{agent,agent_status,pane_id,name}`, and `agent rename <pane-id> <name>`. Read-only observation on Herdr 0.9.0 (macOS) showed a pane whose foreground processes were `node …/pi-profile <name>` and its `pi` child reported as agent `pi`, which is the detection this path relies on; the full launch sequence still requires the owner-authorized manual smoke. Herdr 0.9.0 acknowledges `pane run` with exit 0 and no output at all, so the result layer accepts a silent exit-0 reply for input-only mutations that return no handle (`pane run`, `pane send-text`, `pane send-keys`, `agent send-keys`, `agent rename`); every other operation still requires a JSON success envelope. `pi-profile` 0.1.0 needs no change: everything after the profile name is forwarded to Pi unchanged.

The policy layer uses Pi 0.85.1's mutable/blocking `tool_call` event and argument-array `pi.exec`. macOS and Linux POSIX paths are supported. Windows is not supported by the orchestrator until path invariants and command-guard semantics receive a separate design; the primitive extension remains governed by its existing compatibility contract.
