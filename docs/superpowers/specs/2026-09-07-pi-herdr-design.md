...
- Every workspace, tab, or pane close requires an explicit target ID and `confirm: true`. Reject calls missing either before invoking Herdr. The CLI close commands have no confirmation flag, so enforcement belongs to the extension. This is a trusted-automation contract: `confirm: true` is a deliberate caller assertion, not proof of fresh human authorization. The owner explicitly chose this boundary over per-close human dialogs on 2026-09-07. There is no host-side approval broker or sandbox; callers remain responsible for authorization. Headless callers receive the same deterministic validation.
...
- Bound returned output at 2,000 lines or 50 KiB (51200 UTF-8 bytes), whichever is reached first. Stream subprocess output to temporary storage rather than buffering it without bounds in memory. When truncated, provide a temporary-file location for the full captured output, following pi's documented utilities. The implementation plan must specify artifact handling and precise byte accounting.
...
- Agent state waits, prompt settlement waits, and pane output waits have no default CLI timeout. The extension supplies bounded behavior. Agent startup separately defaults to 30 seconds in the CLI, with a timeout strictly greater than 3000 and at most 300000 milliseconds (confirmed in the installed 0.8.2 API schema).
...
12. Reads and output waits default to `visible`; waits default to 30 seconds and reject nonpositive, nonfinite, or greater-than-five-minute timeouts. Agent startup additionally rejects timeoutMs ≤ 3000 and accepts 3001. Creation and splitting use no-focus by default.
