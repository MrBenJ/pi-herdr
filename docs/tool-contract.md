...
The `HERDR_ALLOW_*` grants are honored consistently across all three layers: `validateTaskInput` (accepting the payload request), `buildWorkerPrompt` (the worker boundary wording), and the `tool_call` guard (permitting the direct `herdr_*` topology tools and relaxing the enqueued-Todo boundary text). A worker is therefore never told a capability is authorized by one layer while another blocks it.
...
While this entry point is loaded, a `tool_call` hook blocks direct `herdr_workspace create`, `herdr_tab create`, `herdr_pane split`, and `herdr_agent start` unless the operator has set `HERDR_ALLOW_WORKSPACES=1` (the same grant that authorizes `allowWorkspaces` — see [Per-run permission flags](#per-run-permission-flags)), so the guard never contradicts a granted worker prompt. Recognizable Bash `git worktree add|move|remove` commands are always blocked regardless of any grant. Read/inspect/prompt/wait and explicitly confirmed close operations retain primitive behavior. `herdr_pane run` and `send-text` are not classified, so a caller holding the primitive tools can still type an agent command into an existing pane; the hook is not a substitute for withholding those tools from workers. The Bash classification is defense in depth, not a shell sandbox.

For `todo add`/`todo update` calls explicitly tagged `enqueue`, the hook requires a prompt, rejects absolute filesystem paths outside the current canonical repository, and appends an idempotent repository boundary. That boundary's "does not authorize creating …" list mirrors the operator env grants — git worktree creation is always listed, Herdr workspaces/tabs/panes/agents only without `HERDR_ALLOW_WORKSPACES=1`, and subagents/background jobs only without `HERDR_ALLOW_DISPATCH=1` — so it stays consistent with the worker prompt and the tool guard. Todo remains tracking-only and no foreign Todo store is opened or mutated.
